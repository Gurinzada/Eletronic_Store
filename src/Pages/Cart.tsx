...
export default function Cart() {
  ...
  const theTotal = () => {
    const total = infosUser?.Cart.reduce(
      (actual, current) => actual + current.price,
      0
    );
  ...
  const removeProduct = async (indexProduct: number) => {
    if (infosUser) {
      const updatedCart = infosUser.Cart.filter(
      ...
      <header className={style.HeaderCart}>
        ...
        <nav>
          <Link to={`/commercial/${UserID}`}>
            <button className={style.Bnt}>Voltar</button>
          </Link>
        </nav>
      </header>
      <main>
        <section className={style.SectionTable}>
          ...
          <table>
            ...
            <tbody>
              {infosUser?.Cart.length || undefined ? (
                infosUser?.Cart.map((user, index) => (
                  <tr key={index}>
